...
def mask_address(address):
    ...
    for city, masked_city in city_patterns.items():
        if re.search(f"{city}[\\s\\S]*", address):
            return masked_city

    for province, masked_province in province_patterns.items():
        if re.search(f"{province}[\\s\\S]*", address):
...
# 매핑 테이블을 원하는 형식으로 출력 (가로 형식)
def create_mapping_dataframe(mapping_table):
    data = {}
    
    for column, mappings in mapping_table.items():
        data[f"{column} (Original)"] = [original for original, _ in mappings]
        data[f"{column} (Anonymized)"] = [anonymized for _, anonymized in mappings]

    # 데이터프레임으로 변환하여 가로로 출력
...
# 리스트박스 크기 설정 및 원래대로 복구
listbox_csv1 = tk.Listbox(frame_csv1, height=25, width=50)  # height와 width 설정 복구
listbox_csv1.grid(row=1, column=0, sticky="nsew")

listbox_csv2 = tk.Listbox(frame_csv2, height=25, width=50)  # height와 width 설정 복구
...
canvas = tk.Canvas(anonymize_tab, width=500, height=400)
